...
namespace JOS.KDTree
{
    ...
    [Serializable]
    public class KdTree<TDimension, TNode> where TDimension : IComparable<TDimension>
    {
        /// <summary>
        ...
        /// <returns>The</returns>
        public IReadOnlyCollection<(TDimension[] Dimensions, TNode Node)> NearestNeighbors(TDimension[] point, int neighbors)
        {
        ...
        public IReadOnlyCollection<(TDimension[] Dimensions, TNode Node)> RadialSearch(TDimension[] center, double radius, int neighboors = -1)
        {
            ...
            IEnumerable<TNode> nodes)
        {
            ...
            if (leftPoints.Length <= 1)
            {
                if (leftPoints.Length == 1)
                {
                    InternalPointArray[BinaryTreeNavigation.LeftChildIndex(index)] = leftPoints[0];
                    InternalNodeArray[BinaryTreeNavigation.LeftChildIndex(index)] = leftNodes[0];
                }
            }
            else
            {
                GenerateTree(BinaryTreeNavigation.LeftChildIndex(index), nextDim, leftPoints, leftNodes);
            }
            ...
            if (rightPoints.Length <= 1)
            {
                if (rightPoints.Length == 1)
                {
                    InternalPointArray[BinaryTreeNavigation.RightChildIndex(index)] = rightPoints[0];
                    InternalNodeArray[BinaryTreeNavigation.RightChildIndex(index)] = rightNodes[0];
                }
            }
            else
            {
                GenerateTree(BinaryTreeNavigation.RightChildIndex(index), nextDim, rightPoints, rightNodes);
            }
        }
